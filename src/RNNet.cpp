...
RNNet::RNNet(int featurenum,int hidlayernum,const char* pathRoot){
    ...
    strcpy(EMOLIST[0],"SENT");
    strcpy(EMOLIST[1],"GENE");
...
Sentence RNNet::FetchSent(const char* pathRoot){
    ...
    free(filelist);
    std::cout<< totnum <<std::endl;
    //std::cout<< totnum <<" "<<genenum<<std::endl;
    return sent;
...
ChUTF8* RNNet::FetchDict(const char* pathRoot){
    ...
    FILE* dictmp=fopen("tmp/dictmp","w");
...
double RNNet::RNNForward_CROSSENTROPY(const char* pathRoot){
    ...
    int pathrootlen=strlen(pathRoot);
    char* psen=(char*)malloc(sizeof(char)*41);
...
double** RNNet::leftDLDGx(){
    long i,j;
    int k;
    j=totnum-genenum;
    Sentence stmp = sent;
    double** ret = (double**)malloc(sizeof(double*)*j);
    for(;j>0;--j){
        double* x = (double*)malloc(sizeof(double)*featureNUM);
